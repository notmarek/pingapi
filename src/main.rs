...

            {
                ...
                if headers.contains_key("Server") {
                    ...
                    if unknown.contains(&status) && server == "cloudflare"
                        || status == forbidden && server == "ddos-guard"
                    {
...
async fn ping_url(url: &String, timeout: u64, socks_ip: &'static str) {
    ...
    // apply headers
    let client = HEADERS_MAP
        .iter()
        .fold(client, |client, header| client.header(*header.0, *header.1));
        ...
        Ok(mut res) => {
            ...
            // separate block for immutable borrow of response
            {
                ...
                if headers.contains_key("Server") {
                    ...
                    let unknown: &[u16] = &[401, 403, 503, 520];
                    let forbidden: &u16 = &403;
                    if status == forbidden && server == "cloudflare" {
                        // likely cf firewall
                        return update_status(
                            ...
                            ping_flaresolverr(
                                ...
                                timeout * 1000, // timeout in ms here
                            )
                            .await
                            .to_str(),
                        );
                    } else if unknown.contains(status) && server.eq("cloudflare")
